...
public:
    ...
    void init(const std::string& hostport) {
        ...
        wsClient.onConnected = [&]() {
            ...
            connectionStatus = "Connected, waiting data...";
        ...
        wsClient.onBinaryMessage = [&](const std::string& msg) {
            ...
            if (start == "MSG") {
                ...
                connectionStatus = "Receiving. " + std::to_string((lastSecondCount * ((int)msg.size())) / 1024) + " KB/sec (" + std::to_string(lastSecondCount) + ")";
                ...
                if (currentModulation == TUNE_REAL && msg.size() == 1024 + REAL_HEADER_SIZE) { // REAL data
                    ...
                    if (scan != msg.data() + REAL_HEADER_SIZE) {
                        ...
                        throw std::runtime_error("KiwiSDRClient: REAL_HEADER_SIZE scan error");
                    }

                    int16_t* ptr = (int16_t*)scan;
                    connectionStatus = "Storing real..";
                    ...
                    iqDataLock.unlock();
                    connectionStatus = "Cont Recv. " + std::to_string((lastSecondCount * ((int)msg.size())) / 1024) + " KB/sec (" + std::to_string(lastSecondCount) + ")";
                }
...
//                flog::info("=> BIN: {} bytes: {}", (int64_t)msg.size(), start);
    ...
    void tune(double freq, Modulation mod) {
        ...
        case Modulation::TUNE_IQ:
            ...
            buf = "SET mod=iq low_cut=-7000 high_cut=7000 freq=" + std::to_string(freq / 1000.0);
            break;
        case Modulation::TUNE_REAL:
            currentModulation = mod;
            buf = "SET mod=usb low_cut=0 high_cut=8000 freq=" + std::to_string((freq - 3000) / 1000.0);
            break;
        }
    ...
    void start() {
        ...
        loop_thread = std::thread([&]() {
            ...
            try {
                ...
                std::string url = "/kiwi/";
                url.append(std::to_string(currentTimeMillis()));
                url.append("/SND");
                wsClient.connectAndReceiveLoop(hostName, port, url);
                connectionStatus = "Disconnected";
                running = false;
            }
    ...
    const std::string& getConnectionStatus() const {
